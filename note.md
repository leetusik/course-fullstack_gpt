...
```
### 10.4~6 Map Re-Rank Chain & Code Challenge
```python
llm = ChatOpenAI(
    temperature=0.1,
)
# template to get answer from each doc in docs.
...
# function that store results of each docs.
...
# prompt for select the best answer.
...
# function for condense all answers and get the best answer among them.
...
# function gets soup object and return str. the str goes to doc.page_content
def parse_page(soup):
...
# get sitemap url and return retriever 
    ...
    if ".xml" not in url:
        ...
        if query:
            chain = (
                ...
                # dict of docs and the questions goes to parameters of get_answers function. 
                | RunnableLambda(get_answers)
                # the output of get_answers(dict) goest to parameters of choose_answer function.
...
# ask first if relative question was there, if it was, then retrieve question's answer from cache.

# no time to do that. maybe later? I mean the feature needed for sure.
...
```python
# If you try to sum of the figures with just plain text, llms often got you wrong answers. It's because llms are built for predicting texts not calculation.
# In order to precise calculation, agent needed to be adopted.
# By giving tool(function), llms can use the tools if the tool is needed. and the point is llm choose to use tools not forced to. 
# So the man give various tools, then llms can choice the tool for the best answer. 
# This called agents. I guess.


```
### 12.2 How do agents work

Agents are work because langchain made prompt that works well. bunch of prompt and looping it till get the result. 
